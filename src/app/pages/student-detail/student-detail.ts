...
/**
 * Student detail page for managing student details
 */
@Component({
...
export class StudentDetailPage {
  ...
  getStudentExamResults(studentId: string) {
    return this.examService
      ...
      .then(results => results.filter(result => result.studentId === studentId));
  }

  getExamName(examId: string) {
    return this.examService
      .getExams()
      .then(exams => exams.find(exam => exam.id === examId)?.name || 'Unknown Exam');
  }
  ...
  updateAnswer(index: number, newAnswer: string) {
    if (this.selectedExamResult) {
      ...
      this.examService
        .updateStudentExamResult(this.selectedExamResult.id, {
          answers: updatedAnswers,
        })
        .then(result => (this.selectedExamResult = result));
    }
  }
