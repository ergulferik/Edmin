import { Component, effect, signal, output, inject, Inject } from "@angular/core";
...
@Component({
  ...
  template: `
  <edmin-modal-header title="Şablon Seç" subtitle="Kullanılacak şablonu seçiniz"></edmin-modal-header>
  <div style="display: flex; flex-direction: row; gap: 1rem; flex-wrap: wrap; justify-content: center; margin: 1rem auto;">
...
export class ExamTemplateListComponent {
  ...
  constructor(private examService: ExamService, @Inject(MAT_DIALOG_DATA) public data: { selectedTemplate: ExamTemplate }) {
    effect(async () => {
  ...
  selectTemplate(template: ExamTemplate): void {
    ...
    if(this.examService.getSelectedExam()) {
      this.examService.setSelectedExam({...this.examService.getSelectedExam()!, templateId: template.id});
    }
